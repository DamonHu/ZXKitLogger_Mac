...
public extension Data {
    ...
    func encodeString(encodeType: ZXKitUtilEncodeType) -> String? {
        switch encodeType {
            ...
            case .system(let encode):
                return String(data: self, encoding: encode)
        }
    }
    ...
    func hashString(hashType: ZXKitUtilHashType, lowercase: Bool = true) -> String {
        ...
        switch hashType {
            case .md5:
                ...
                _ = self.withUnsafeBytes { (messageBytes) -> Bool in
...
public extension  Data {
    ...
    func aesGCMEncrypt(key: SymmetricKey, encodeType: ZXKitUtilEncodeType = .base64, nonce: AES.GCM.Nonce? = AES.GCM.Nonce()) -> String? {
        ...
        guard let sealedBox = try? AES.GCM.seal(self, using: key, nonce:  nonce) else { return nil }
        guard let encode = sealedBox.combined else { return nil }
        return encode.encodeString(encodeType: encodeType)
    }
    //AES GCM解密
    ...
    func aesGCMDecrypt(key: SymmetricKey) -> String? {
        assert(key.bitCount / 8 == kCCKeySizeAES128 || key.bitCount / 8 == kCCKeySizeAES192 || key.bitCount / 8 == kCCKeySizeAES256, "Invalid key length. Available length is \(kCCKeySizeAES128) \(kCCKeySizeAES192) \(kCCKeySizeAES256)")
        guard let sealedBox = try? AES.GCM.SealedBox.init(combined: self) else { return nil }
    ...
    func hmac(hashType: ZXKitUtilHashType, key: SymmetricKey, encodeType: ZXKitUtilEncodeType = .base64) -> String? {
        switch hashType {
            case .md5:
                let sign = HMAC<Insecure.MD5>.authenticationCode(for: self, using: key)
                return Data(sign).encodeString(encodeType: encodeType)
            case .sha1:
                let sign = HMAC<Insecure.SHA1>.authenticationCode(for: self, using: key)
                return Data(sign).encodeString(encodeType: encodeType)
            case .sha224:
                assert(false, "unsupported hash type")
                return nil
            case .sha256:
                let sign = HMAC<SHA256>.authenticationCode(for: self, using: key)
                return Data(sign).encodeString(encodeType: encodeType)
            case .sha384:
                let sign = HMAC<SHA384>.authenticationCode(for: self, using: key)
                return Data(sign).encodeString(encodeType: encodeType)
            case .sha512:
                let sign = HMAC<SHA512>.authenticationCode(for: self, using: key)
                return Data(sign).encodeString(encodeType: encodeType)
        }
    }
}
